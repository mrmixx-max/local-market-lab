...
os.environ["LML_DB_PATH"] = os.path.join(tempfile.mkdtemp(prefix="lml_beta_"), "beta.db")
...
try:
    ...
    bars = [PriceBar(d, c_) for d, c_ in [("2024-01-02", 100.0), ("2024-01-03", 101.0),
                                          ("2024-01-08", 102.0)]]
...
try:
    ...
    px, v = [], 100.0
    for _ in range(400):
        v *= 1 + rng.gauss(0.0004, 0.012); px.append(v)
    ...
    record("backtest-costs", cost["curve"][-1] < free["curve"][-1],
...
try:
    ...
    r = walk_forward_backtest(px, lambda tr, te: [1.0]*len(te), train_window=200,
                              test_window=50, step=25, seed=42)
    record("walk-forward", r.n_folds > 0 and r.summary()["seed"] == 42, f"folds={r.n_folds}")
except Exception as e:
...
try:
    ...
    r = hyperparameter_tune(lambda tr, te, **p: [p["w"]]*len(te), px,
                            {"w": [0.5, 1.0]}, n_trials=2, seed=42, method="grid")
    record("hyperparameter-tuning", r.n_trials == 2 and r.best_params in ({"w": .5}, {"w": 1.0}))
except Exception as e:
...
try:
    ...
    record("stress-tests", a.run_id == b.run_id and "p01" in mc, f"dd={a.metrics['max_drawdown']}")
except Exception as e:
...
try:
    ...
    no_exec = not any(w in src.lower() for w in ("place_order", "submit_order", "broker"))
    record("rebalancing-proposals-only", len(r.proposals) > 0 and no_exec,
...
try:
    ...
    record("api-health+traversal", ok_health and ok_trav,
