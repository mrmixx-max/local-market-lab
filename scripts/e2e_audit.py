...
# ---------- data hash ----------
step("data_hash", lambda: {
    "sha256": hashlib.sha256(json.dumps([DATES, PX], sort_keys=True).encode()).hexdigest()[:16]})
...
def _quality():
    ...
    return {"points": q["points"], "dupes": dupes, "issues": len(issues),
...
def _features():
    ...
    return {"n_returns": int(len(rets)), "mean": float(rets.mean()),
            "std": float(rets.std())}
...
def _tune():
    ...
    r = hyperparameter_tune(
        lambda tr, te, **p: [p.get("w", 1.0)]*len(te), PX,
        {"w": [0.5, 1.0, 1.5]}, n_trials=3, seed=SEED, method="grid")
    return r.summary()
...
def _compare():
    ...
    mk = lambda name, noise: [
        WalkForwardResult(window=w, train_start=0, train_end=w, test_start=w,
                          test_end=w+63, model_name=name,
...
def _explain():
    ...
    X = rng2.standard_normal((200, 4)); y = X[:, 1]*3 + rng2.standard_normal(200)*0.2
    pi = permutation_importance(lambda X_: X_[:, 1]*3, X, y, n_repeats=5, seed=SEED).to_dict()
    sh = shapley_approx(lambda X_: X_[:, 1]*3, X, X[0], n_samples=30, seed=SEED)
    return {"top_feature": max(pi["feature_importance"], key=lambda f: f["importance"])["feature"],
...
def _backtest():
    ...
    pr = run_backtest(prices, PeriodicRebalance(63), Assumptions(fees_bps=10, slippage_bps=5))
    return {"bh_final": round(bh["curve"][-1], 6), "pr_final": round(pr["curve"][-1], 6),
            "bh_trades": bh["trades"], "pr_trades": pr["trades"],
            "costs_less_than_free": pr["curve"][-1] < bh["curve"][-1]}
step("backtest_costs", _backtest)

# ---------- stress + rebalancing ----------
def _stress():
    from packages.scenarios.stress import (HISTORICAL_CRISES, HYPOTHETICAL_SCENARIOS,
                                           monte_carlo_fat_tail, run_historical_stress,
...
def _rebal():
    ...
    r = suggest_rebalance({"IWDA": 0.72, "AGGH": 0.28}, {"IWDA": 0.6, "AGGH": 0.4},
                          transaction_cost_bps=10)
    return {"proposals": [{"symbol": p.symbol, "action": p.action} for p in r.proposals],
...
def _exports():
    ...
    x = excel_report({"cagr_pct": 7.1}, [{"symbol": "DEMO", "qty": 10}], curve, dq=dq, seed=SEED)
    ...
    vals = {wb["Quality"].cell(row=i, column=1).value: wb["Quality"].cell(row=i, column=2).value
            for i in range(1, 16)}
    assert vals.get("system_version") and vals.get("seed") == SEED
    assert open(p.file_path, "rb").read(4) == b"%PDF"
    return {"csv_hash": c.data_hash, "xlsx_hash": x.data_hash, "pdf_hash": p.data_hash,
            "csv_meta_ok": True, "xlsx_meta_ok": True, "pdf_magic_ok": True}
