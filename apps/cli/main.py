...
app = typer.Typer(
    help="Local Market Lab — research tool, no investment advice.", no_args_is_help=True
)
...
def demo(db: str = DB_OPT):
    ...
    for strat in (BuyAndHold(), PeriodicRebalance(63)):
        ...
        m = r["metrics"]
        typer.echo(
            f"{strat.name:<22} CAGR={m['cagr_pct']:>7}%  MaxDD={m['max_drawdown_pct']:>6}%  "
            f"Sharpe={m['sharpe']:>6}"
        )
...
def backtest(
    ...
    result = backtest_from_workspace(
        ws, name, strat, Assumptions(fees_bps=fees_bps, slippage_bps=slippage_bps)
    )
...
def scenario(
    ...
    if kind == "mc":
        ...
        manifest = build_manifest(
            "replay", {"symbols": symbols}, None, None, {"symbols": symbols}
        )
        save(ws, manifest)
...
def doctor(db: str = DB_OPT):
    ...
    typer.echo(
        f"instruments={n_instr} transactions={n_txn} price_rows={n_prices} "
...
def import_market(
    ...
    typer.echo(
        f"imported {result['points']} bars for {result['symbol']} "
...
def game_create(
    symbols: str = "IWDA,EIMI,AGGH", days: int = 63, seed: int = 42, db: str = DB_OPT
):
    ...
    typer.echo(
        f"game_id={g.game_id} status={g.status.value} "
        f"symbols={g.symbols} days={g.total_days}"
    )


@game_app.command("order")
def game_order(game_id: str, symbol: str, side: str, quantity: float, db: str = DB_OPT):
...
def game_tick(game_id: str, days: int = 1, db: str = DB_OPT):
    ...
    for _ in range(days):
        ...
        typer.echo(
            f"day={state['day']} value={state['total_value']:,.2f} "
...
def game_leaderboard(db: str = DB_OPT):
    ...
    for i, entry in enumerate(lb, 1):
        typer.echo(
            f"{i}. {entry['player']:<16} {entry['challenge']:<20} "
