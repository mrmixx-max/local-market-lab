    ...
    if chart_images:
        ...
        for img in chart_images:
            try:
                ...
                bio.seek(0)
                elements.extend([Image(bio, width=160 * mm, height=90 * mm), Spacer(1, 4 * mm)])
            except Exception:
...
def excel_report(metrics: dict, trades: list[dict],
    ...
    with open(fp, "wb") as f:
        f.write(raw)
    return ExportResult(run_id=run_id, format="excel", data_quality=dq,
                        data_hash=_data_hash(raw), file_path=fp, file_bytes=raw,
                        created_at=_utc(), metadata={"sheets": wb.sheetnames})
