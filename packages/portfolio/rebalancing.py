...
DISCLAIMER = ("Dies ist ein Analyseergebnis. Keine automatische Orderausführung, "
...
def suggest_rebalance_orders(
    ...
    fee_bps = _env_float("LML_REBALANCE_FEE_BPS", fee_bps if fee_bps is not None
                         else _REBALANCE_ENV_DEFAULTS["LML_REBALANCE_FEE_BPS"])
    min_fee = _env_float("LML_REBALANCE_MIN_FEE", min_fee if min_fee is not None
                         else _REBALANCE_ENV_DEFAULTS["LML_REBALANCE_MIN_FEE"])
    spread_bps = _env_float("LML_REBALANCE_SPREAD_BPS", spread_bps if spread_bps is not None
                            else _REBALANCE_ENV_DEFAULTS["LML_REBALANCE_SPREAD_BPS"])
    allow_fractional = _env_bool("LML_REBALANCE_ALLOW_FRACTIONAL",
                                 allow_fractional if allow_fractional is not None
                                 else _REBALANCE_ENV_DEFAULTS["LML_REBALANCE_ALLOW_FRACTIONAL"])
    min_order_strategy = (min_order_strategy or os.environ.get(
        "LML_REBALANCE_MIN_ORDER_STRATEGY",
        _REBALANCE_ENV_DEFAULTS["LML_REBALANCE_MIN_ORDER_STRATEGY"])).lower()
    default_min = _env_float("LML_REBALANCE_DEFAULT_MIN_ORDER_VALUE",
                             default_min_order_value if default_min_order_value is not None
    ...
    positions_value = {s: max(0.0, p["quantity"]) * p["price"]
                       for s, p in current_positions.items()}
    total_value = cash + sum(positions_value.values())
    if total_value <= 0:
        return RebalanceOrdersResult(
            run_id="", data_hash="", cash_before=cash, cash_after=cash,
            total_fees_estimate=0.0, orders_skipped_below_minimum=0,
            cost_benefit_status="not_worthwhile",
            warnings=["Portfolio has zero or negative total value."],
            disclaimer=DISCLAIMER, proposals=[], needs_rebalance=False,
            drift_threshold=threshold, drift_analysis=[])

    current_weights = {s: positions_value[s] / total_value
    ...
    for d in drift:
        ...
        if not allow_fractional and raw_qty != 0:
            ...
            if rounded != raw_qty:
                rounding_note = (f"{sym}: rounded {raw_qty:.4f} → {rounded} shares; "
        ...
        if below_min:
            ...
            proposals.append(OrderProposal(
                symbol=sym, current_weight=round(d.current_weight, 4),
                target_weight=round(d.target_weight, 4), drift=round(d.drift_abs, 4),
                raw_order_quantity=round(raw_qty, 4),
                adjusted_order_quantity=0.0, order_value=0.0,
                min_order_size=round(min_size, 2), below_minimum=True,
                fees_estimate=0.0, rounding_note=rounding_note, action=action))
            continue
        ...
        if action == "buy":
            ...
            if needed > cash_after:
                ...
                order_value = adjusted_qty * price
                fees = max(min_fee, order_value * fee_bps / 10000) + order_value * spread_bps / 10000
                needed = order_value + fees
                warnings.append(f"{sym}: buy capped to available cash (no negative cash)")
                if adjusted_qty <= 0:
                    skipped += 1
                    proposals.append(OrderProposal(
                        symbol=sym, current_weight=round(d.current_weight, 4),
        ...
        total_fees += fees
        total_benefit += d.drift_abs * drift_cost_per_day_bps * holding_period_days / 100
        proposals.append(OrderProposal(
            symbol=sym, current_weight=round(d.current_weight, 4),
            target_weight=round(d.target_weight, 4), drift=round(d.drift_abs, 4),
            raw_order_quantity=round(raw_qty, 4),
            adjusted_order_quantity=round(adjusted_qty, 4),
            order_value=round(order_value, 2), min_order_size=round(min_size, 2),
            below_minimum=False, fees_estimate=round(fees, 2),
            rounding_note=rounding_note, action=action))
    ...
    if total_fees <= 0 and total_benefit > 0:
        ...
        status = "worthwhile" if ratio >= 2 else ("marginal" if ratio >= 1 else "not_worthwhile")

    cash_after = max(0.0, round(cash_after, 2))
    # deterministic run identity
    ident = _canonical_hash({
        "positions": current_positions, "targets": target_weights, "cash": cash,
        "threshold": threshold, "fee_bps": fee_bps, "min_fee": min_fee,
        "spread_bps": spread_bps, "allow_fractional": allow_fractional,
        "strategy": min_order_strategy, "default_min": default_min,
        "overrides": min_order_overrides, "seed": seed,
    })
    run_id = ident[:12]

    return RebalanceOrdersResult(
        run_id=run_id, data_hash=ident, cash_before=round(cash, 2),
        cash_after=cash_after, total_fees_estimate=round(total_fees, 2),
        orders_skipped_below_minimum=skipped, cost_benefit_status=status,
        warnings=warnings, disclaimer=DISCLAIMER, proposals=proposals,
        needs_rebalance=len(proposals) > 0,
        drift_threshold=threshold, drift_analysis=drift)
...
def rebalance_orders_from_valuation(
    ...
    **kwargs,
    ...
    return suggest_rebalance_orders(
        current, target_weights, cash, threshold,
        min_order_overrides=min_order_overrides, **kwargs)
