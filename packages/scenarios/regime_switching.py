...
def detect_regime(returns, n_regimes: int = 3) -> dict:
    ...
    names = (["Trend", "Seitwaerts", "Volatil"] + [f"Regime_{i}" for i in range(3, n_regimes)])[:n_regimes]
    return {"labels": labels.tolist(), "means": [round(float(v), 6) for v in mu],
            "stds": [round(float(v), 6) for v in sigma], "weights": [round(float(v), 4) for v in pi],
            "regime_names": names, "n_regimes": n_regimes}
...
def regime_forecast(data, horizon: int = 30) -> dict:
    ...
    models = [_momentum, _mean_reversion, _garch_like] if reg["n_regimes"] == 3 else [_momentum] * reg["n_regimes"]
    ...
    return {"model": "regime_switching", "current_regime": names[cur], "current_regime_id": cur,
            "forecast": forecast,
...
def regime_probability(data) -> dict:
    ...
    rets = np.diff(arr)
    reg = detect_regime(rets)
    mu, sigma, pi_w = np.array(reg["means"]), np.array(reg["stds"]), np.array(reg["weights"])
    k = reg["n_regimes"]
    names = reg["regime_names"]
    inst = np.array([pi_w[j] * _gauss(np.array([float(rets[-1])]), mu[j], sigma[j])[0] for j in range(k)])
