...
def correlation_break(
    ...
    def port_var(rho: float) -> float:
        return sum(
            weights[i] * weights[j] * vols[i] * vols[j] * (1.0 if i == j else rho)
            for i in range(n)
            for j in range(n)
        )
    ...
    return CrisisResult(
        ...
        round(-delta_var, 2),
        {
            "vol_normal_annual_pct": round(vol_n, 2),
            "vol_crisis_annual_pct": round(vol_c, 2),
            "diversification_lost_pct": (
                round((1 - vol_c / vol_n) * 100, 1) if vol_n > 0 else 0
            ),
        },
        [
            "Reduce position sizes.",
            "Add diversifiers: trend-following, long vol.",
...
def liquidity_crunch(
    ...
    for sym, value in positions.items():
        ...
        penalty = (
            math.sqrt(participation / participation_cap)
            if participation > participation_cap
            else math.sqrt(participation)
        )
    ...
    return CrisisResult(
        "liquidity_crunch",
        round(-total_cost_pct, 3),
        {
            "assumed_daily_adv_fraction": adv_fraction,
            "participation_cap": participation_cap,
            "position_impacts_pct": impacts,
        },
        [
            "Stagger liquidation over multiple days.",
            "Prefer liquid instruments.",
            "Maintain cash buffer.",
        ],
        [
            "Square-root model is a simplification.",
            "ADV may not reflect stressed markets.",
        ],
    )


def sector_rotation(
    positions: dict[str, float], rotation_shift: dict[str, float] | None = None
) -> CrisisResult:
    ...
    return CrisisResult(
        ...
        round(total * 100, 2),
        {
            "sector_exposures_pct": {
                s: round(e * 100, 1) for s, e in sector_exp.items()
            },
            "impact_by_sector_pct": impacts,
            "rotation_shifts_applied": rotation_shift,
        },
