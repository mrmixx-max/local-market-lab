...
def import_transactions(
    ws, path: str | Path, portfolio: str, default_currency: str = "EUR"
) -> dict:
    ...
    for i, row in enumerate(rows, start=2):
        try:
            ...
            qty = float(
                (_pick(row, ["quantity", "menge", "stück", "stueck"]) or "0").replace(
                    ",", "."
                )
            )
...
def import_prices(ws, path: str | Path, symbol: str, source: str = "user-csv") -> dict:
    ...
    for i, row in enumerate(rows, start=2):
        try:
            ...
            close = float(
                (_pick(row, ["close", "schluss", "preis"]) or "").replace(",", ".")
            )
    ...
    for i, row in enumerate(rows, start=2):
        try:
            ca = CorporateAction(
                ...
                ratio=(
                    float(r)
                    if (r := (_pick(row, ["ratio"])) or "").replace(",", ".")
                    else None
                ),
                amount_per_share=(
                    float(a)
                    if (
                        a := (_pick(row, ["amount_per_share", "betrag"])) or ""
                    ).replace(",", ".")
                    else None
                ),
                currency=(_pick(row, ["currency", "währung", "waehrung"]) or "EUR")
                .strip()
                .upper(),
            )
