...
def check_missing_data(
    series: PriceSeries, max_gap_days: int = 5
) -> tuple[int, list[str]]:
    ...
    for a, b in zip(dates, dates[1:]):
        gap = (b - a).days
        weekdays = sum(
            1 for i in range(1, gap) if (a + timedelta(days=i)).weekday() < 5
        )
...
def check_splits(
    series: PriceSeries, threshold: float = 0.4
...
def check_fx_consistency(
    series: PriceSeries, expected_ccy: str = "USD"
) -> tuple[bool, list[str]]:
    """Flag if series currency mismatches expected."""
    issues = []
    if series.currency and series.currency != expected_ccy:
        issues.append(
            f"currency mismatch: series={series.currency}, expected={expected_ccy}"
        )
...
def detect_outliers(
    series: PriceSeries, z_threshold: float = 4.0
) -> tuple[int, list[dict]]:
...
def run_quality_check(
    ...
    if missing_count / max(n, 1) > MISSING_THRESHOLD:
        issues.append(
            f"missing data: {missing_count} bars exceed {MISSING_THRESHOLD*100:.0f}%"
        )
        status = "warning"
    if dupes > 0:
    ...
    penalties = [
        min(missing_ratio * 2, 0.3),
        0.2 if split_detected else 0.0,
        0.15 if fx_mismatch else 0.0,
        min(len(ts_issues) * 0.05, 0.3),
        0.3 if dupes > 0 else 0.0,
        0.1 if stale else 0.0,
        min(outlier_count * 0.03, 0.25),
    ]
    score = max(0.0, 1.0 - sum(penalties))
    data_hash = hashlib.sha256(
        "".join(b.date + str(b.close) for b in series.bars).encode()
    ).hexdigest()[:16]
