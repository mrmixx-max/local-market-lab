...
class TestVersionedCache:
    ...
    def test_schema_change_invalidates(self, tmp_path):
        c = MarketDataCache(tmp_path / "c.db")
        old_key = (
            "adjusted=True|currency=USD|interval=1d|provider=yahoo|schema=1|symbol=AAPL"
        )
...
class TestRetryBackoff:
    def test_exhaustion_raises_ratelimit(self, tmp_path, monkeypatch):
        ...
        a = _DummyAdapter(
            cache_path=str(tmp_path / "c.db"), max_retries=2, retry_base_delay=0
        )
        calls = {"n": 0}
        ...
        def fake_urlopen(req, timeout=30):
            ...
            raise urllib.error.HTTPError(
                req.full_url, 429, "slow down", hdrs=None, fp=None
            )
    ...
    def test_http_404_raises_immediately(self, tmp_path, monkeypatch):
        ...
        a = _DummyAdapter(
            cache_path=str(tmp_path / "c.db"), max_retries=3, retry_base_delay=0
        )
        calls = {"n": 0}

        def fake_urlopen(req, timeout=30):
            calls["n"] += 1
            raise urllib.error.HTTPError(req.full_url, 404, "nope", hdrs=None, fp=None)
