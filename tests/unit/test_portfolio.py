...
class TestBuildPositions:
    def test_buy_increases_qty(self, ws):
        ws.add_transaction({"portfolio": "p1", "symbol": "TEST", "txn_type": "buy",
                            "date": "2026-01-01", "quantity": 10, "price": 100,
                            "fees": 5, "currency": "EUR", "note": ""})
        pos = build_positions(ws, "p1")
        assert "TEST" in pos
        assert abs(pos["TEST"].quantity - 10.0) < 1e-9

    def test_sell_consumes_fifo_lots(self, ws):
        ws.add_transaction({"portfolio": "p", "symbol": "TEST", "txn_type": "buy",
                            "date": "2026-01-01", "quantity": 10, "price": 100,
                            "fees": 0, "currency": "EUR", "note": ""})
        ws.add_transaction({"portfolio": "p", "symbol": "TEST", "txn_type": "buy",
                            "date": "2026-01-02", "quantity": 10, "price": 110,
                            "fees": 0, "currency": "EUR", "note": ""})
        ws.add_transaction({"portfolio": "p", "symbol": "TEST", "txn_type": "sell",
                            "date": "2026-01-03", "quantity": 5, "price": 120,
                            "fees": 0, "currency": "EUR", "note": ""})
...
class TestValuePortfolio:
    def test_basic_valuation(self, ws):
        ws.add_transaction({"portfolio": "p", "symbol": "TEST", "txn_type": "buy",
                            "date": "2026-01-01", "quantity": 10, "price": 90,
                            "fees": 0, "currency": "EUR", "note": ""})
    ...
    def test_missing_fx_incomplete(self, ws):
        ...
        import_prices(ws, _price_csv(Path(ws.db_path).parent, "USD1", [100.0]*10), "USD1", "test")
        ws.add_transaction({"portfolio": "p", "symbol": "USD1", "txn_type": "buy",
                            "date": "2026-01-01", "quantity": 1, "price": 100,
