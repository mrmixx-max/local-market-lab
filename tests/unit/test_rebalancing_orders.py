...
class TestCash:
    ...
    def test_no_negative_position_sell_capped(self):
        ...
        res = suggest_rebalance_orders(
            pos, {"A": 0.0}, cash=0, threshold=0.001, default_min_order_value=0.0
        )
        if res.proposals:
            a = res.proposals[0]
...
class TestMultipleSymbols:
    def test_different_min_sizes(self):
        ...
        res = suggest_rebalance_orders(
            ...
            cash=3000,
            threshold=0.001,
            min_order_overrides={"A": 500.0, "B": 20.0, "C": 20.0},
...
class TestReproducibility:
    def test_same_input_same_output(self):
        ...
        targets = {"A": 0.4, "B": 0.6}
        r1 = suggest_rebalance_orders(
            _positions(pos_spec), targets, cash=500, threshold=0.001, seed=42
        )
        r2 = suggest_rebalance_orders(
            _positions(pos_spec), targets, cash=500, threshold=0.001, seed=42
        )
