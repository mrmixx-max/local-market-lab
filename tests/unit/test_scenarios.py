...
class TestRegimeSwitching:
    ...
    def test_gmm_fit_stability(self):
        ...
        rng = np.random.RandomState(42)
        x = np.concatenate(
            [rng.normal(-3, 0.2, 67), rng.normal(0, 0.2, 67), rng.normal(3, 0.2, 66)]
        )
...
class TestOnlineEnsemble:
    ...
    def test_drift_mean_shift(self):
        ...
        data = np.concatenate(
            [np.random.normal(100, 1, 100), np.random.normal(110, 1, 100)]
        ).tolist()
        r = drift_detection(data)
    ...
    def test_drift_volatility_shift(self):
        ...
        rng = np.random.RandomState(0)
        data = np.concatenate(
            [rng.normal(100, 0.5, 100), rng.normal(100, 3, 100)]
        ).tolist()
        r = drift_detection(data)
        assert r["drift_detected"]
...
class TestDeepLearning:
    ...
    def test_walk_forward_lstm(self, price_data):
        from packages.scenarios.deep_learning import (
            lstm_forecast,
...
class TestTransformer:
    ...
    def test_walk_forward_transformer(self, price_data):
        from packages.scenarios.transformer_forecast import (
