...
HISTORICAL_CRISES: dict[str, dict[str, float]] = {
    "2008_financial_crisis": {
        "equity": -0.57, "bond_gov": 0.12, "bond_corp": -0.15,
        "commodity": -0.55, "real_estate": -0.40, "cash": 0.02,
        "description": "Global Financial Credit Crisis (Lehman collapse)",
    },
    "2020_covid_crash": {
        "equity": -0.34, "bond_gov": 0.04, "bond_corp": -0.10,
        "commodity": -0.45, "real_estate": -0.20, "cash": 0.005,
        "description": "COVID-19 Pandemic Crash (Feb-Mar 2020)",
    },
    "2022_inflation_shock": {
        "equity": -0.25, "bond_gov": -0.18, "bond_corp": -0.16,
        "commodity": 0.20, "real_estate": -0.15, "cash": 0.015,
...
HYPOTHETICAL_SCENARIOS: dict[str, dict[str, float]] = {
    "crash_30pct": {
        "equity": -0.30, "bond_gov": 0.05, "bond_corp": -0.05,
        "commodity": -0.20, "real_estate": -0.15, "cash": 0.01,
        "description": "Sudden equity crash -30%, flight to quality",
    },
    "volatility_spike": {
        "equity": -0.15, "bond_gov": -0.02, "bond_corp": -0.08,
        "commodity": 0.10, "real_estate": -0.10, "cash": 0.01,
        "description": "Volatility spike: equity -15%, correlation breakdown",
    },
    "rates_300bp": {
        "equity": -0.10, "bond_gov": -0.20, "bond_corp": -0.15,
        "commodity": 0.05, "real_estate": -0.25, "cash": 0.03,
...
def _recovery_months(total: float) -> int | None:
    if total >= 0:
        return 0
    return math.ceil(math.log(1 / (1 + total)) / math.log(1.02)) if (1 + total) > 0 else None
...
def _apply_shocks(scenario_name: str, shocks: dict, positions: dict,
    ...
    timeline = [
        ...
        {"day": 1, "portfolio_value_pct": round(100 * (1 + total), 2),
         "event": "shock_applied"},
    ]
    if months:
        for m in [months // 4, months // 2, months]:
            if m > 0:
                timeline.append({"day": m * 30,
                                 "portfolio_value_pct": round(
                                     100 * (1 + total) * (1.02 ** m), 2),
    ...
    return StressTestResult(
        ...
        timeline=timeline, data_hash=_data_hash(positions),
        limitations=[
...
def run_historical_stress(scenario_name: str, positions: dict[str, float],
                           seed: int = 42) -> StressTestResult:
...
def available_scenarios() -> dict:
    return {
        ...
        "hypothetical": {k: v["description"] for k, v in HYPOTHETICAL_SCENARIOS.items()},
    }
